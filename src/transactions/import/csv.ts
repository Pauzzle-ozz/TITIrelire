...
export interface CsvMapping {
  ...
  amount?: string
  /** Column holding debit (money out) — used with `credit` for bank-style exports. */
  debit?: string
  /** Column holding credit (money in) — used with `debit`. */
  credit?: string
  label?: string
  currency?: string
  /** Column holding a stable id. When absent, a deterministic id is derived. */
  id?: string
  counterparty?: string
  category?: string
}
...
export interface CsvOptions {
  ...
  delimiter?: string
  /** Decimal separator. Auto-detected per value when omitted. */
  decimal?: '.' | ','
...
/** Parses an amount string, tolerating thousands separators, comma decimals and `(…)` negatives. */
...
/** Converts a source date to ISO `YYYY-MM-DD` (validation happens in normalize). */
...
/** Small deterministic hash (djb2) so generated ids are stable across imports. */
...
export function importCsv(text: string, mapping: CsvMapping, options: CsvOptions = {}): Transaction[] {
  ...
  const raws: RawTransaction[] = rows.slice(1).map((row, rowIndex) => {
    ...
    const id =
      idCol !== undefined && cell(row, idCol) !== ''
        ? cell(row, idCol)
        : `${source}-${hash(`${date}|${amount}|${label}|${rowIndex}`)}`
